...
<main class="min-h-screen bg-gradient-to-br from-[#9FB1A3] to-[#7F8F83] font-jost">
    <div class="max-w-7xl mx-auto py-6 px-4">
        ...
        <button id="mobilSidebarToggle" type="button"
            class="lg:hidden fixed top-4 left-4 z-50 bg-white p-3 rounded-full shadow-lg">
        ...
        <div class="mb-8">
            <div class="flex flex-col sm:flex-row gap-4 mb-4">
                <button id="szuroGomb" type="button"
                    class="bg-white px-6 py-3 rounded-xl shadow-lg flex items-center gap-2 font-semibold text-[#4A7043] hover:bg-gray-50 transition w-full sm:w-auto">
                    <span>Szűrő</span>
                    <span id="szuroSzamlalo"
                        class="hidden bg-[#6F837B] text-white text-xs px-2 py-1 rounded-full">0</span>
                    <svg id="szuroNyil" class="w-5 h-5 transition-transform" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                ...
                <a href="hutom.php"
                    class="bg-white px-6 py-3 rounded-xl shadow-lg flex items-center gap-2 font-semibold text-[#4A7043] hover:bg-gray-50 transition w-full sm:w-auto justify-center">
            ...
            <div id="szuroPanel" class="hidden mt-4 bg-white rounded-2xl shadow-2xl p-6">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button id="szuroReset" type="button"
                        class="text-xs text-red-600 hover:text-red-800 font-medium">Szűrők alaphelyzetbe</button>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <label class="inline-block cursor-pointer">
                            <input type="checkbox" class="arkategoriaCheckbox peer sr-only"
                                data-arkategoria="<?= htmlspecialchars($arKat) ?>">
                            <span
                                class="inline-block px-4 py-2 rounded-xl bg-gray-100 text-gray-600 text-sm font-medium transition peer-checked:bg-[#6F837B] peer-checked:text-white">
                ...
                <div class="mb-6 max-w-xs">
                    <label for="idoSzuro"
                        class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Elkészítési
                        idő</label>
                    <select id="idoSzuro"
                        class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-[#6F837B]/40 focus:border-[#6F837B]">
        ...
        <div class="flex flex-col lg:flex-row lg:gap-8">
            ...
            <aside id="sidebar"
                class="fixed inset-y-0 left-0 z-40 w-80 bg-white shadow-2xl transform -translate-x-full lg:translate-x-0 lg:sticky lg:top-6 lg:self-start lg:max-h-[calc(100vh-3rem)] lg:overflow-y-auto lg:w-[280px] transition-transform duration-300 rounded-r-2xl lg:rounded-2xl">
                <div class="p-5">
                    ...
                    <div class="mb-4">
                        <button type="button"
                            ...
                            <svg class="w-5 h-5 transition-transform sav-nyil" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 9l-7 7-7-7" />
                            </svg>
                            ...
                            <li class="sidebar-recept-item" data-recept-id="<?= (int)$r['ReceptID'] ?>">
                                ...
                                <a href="receptek.php?id=<?= (int)$r['ReceptID'] ?>&szint=<?= $rSzint ?>"
                                    ...
                                    <a href="javascript:void(0)"
                                        ...
                                        <div class="text-xs opacity-80"><?= formatIdo((string)$r['ElkeszitesiIdo']) ?>
                                        </div>
                                    </a>
                            </li>
                            <?php endforeach; ?>
            ...
            <section id="receptekTarolo" class="flex-1">
                ...
                <div class="mb-8 bg-white/80 backdrop-blur px-5 py-4 rounded-2xl shadow-xl">
                    <div class="flex items-center justify-between gap-3">
                        <div class="font-bold text-[#4A7043]">Jelenlegi szint: <?= (int)$progress['aktualisSzint'] ?>
                        </div>
                ...
                <div class="mb-16 szint-blokk" data-szint="<?= (int)$szint ?>">
                    <h2 class="text-3xl font-bold text-white mb-6 border-b border-white/40 pb-3">
                        <?= (int)$szint ?>. szint – <span class="szint-darab-fo"
                            data-szint="<?= (int)$szint ?>"><?= count($lista) ?></span> recept
                    </h2>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <a href="receptek.php?id=<?= (int)$r['ReceptID'] ?>&szint=<?= $rSzint ?>"
                            ...
                            <div class="relative">
                                <img src="<?= htmlspecialchars(receptKepSrc($r['Kep'] ?? '')) ?>"
                            ...
                            <div class="p-5">
                                <div class="flex justify-between text-xs text-gray-500 mb-2 font-semibold">
                                    <span
                                        class="text-[#6F837B] uppercase"><?= htmlspecialchars($r['AlkategoriaNev'] ?? 'Egyéb') ?></span>
                ...
                <div class="mt-10 flex justify-center">
                    <ul id="szintNav"
                        ...
                        <li>
                            <button type="button" class="szint-nav-gomb focus:outline-none"
                                data-szint="<?= (int)$szintNav ?>" aria-label="<?= (int)$szintNav ?>. szint receptjei">
                                <span data-role="pill"
                ...
                <?php
                    ...
                    ?>
                <a href="<?= htmlspecialchars($visszaUrl) ?>"
                ...
                <div class="bg-white rounded-3xl shadow-2xl overflow-hidden">
                    <div class="relative h-80">
                        <img src="<?= htmlspecialchars(receptKepSrc($recept['Kep'] ?? '')) ?>"
                        ...
                        <div class="absolute bottom-6 left-8 text-white">
                            ...
                            <div class="flex flex-wrap gap-3 text-sm">
                                <span class="px-3 py-1 bg-white/20 backdrop-blur-md rounded-full">⏱
                                    <?= formatIdo((string)($recept['ElkeszitesiIdo'] ?? '')) ?></span>
                                <span class="px-3 py-1 bg-white/20 backdrop-blur-md rounded-full">⭐
                                    <?= (int)($recept['BegyujthetoPontok'] ?? 0) ?> pont</span>
                                <span class="px-3 py-1 bg-white/20 backdrop-blur-md rounded-full">🔥
                                    <?= formatMennyiseg((float)($recept['Kaloria'] ?? 0)) ?> kcal</span>
                                <span class="px-3 py-1 bg-white/20 backdrop-blur-md rounded-full">🏷
                                    <?= htmlspecialchars($recept['AlkategoriaNev'] ?? '') ?></span>
                            </div>
                        </div>
                    </div>
                            ...
                            <ul class="space-y-3 text-sm">
                                ...
                                <li class="flex gap-2 items-start">
                                    ...
                                    <span class="font-bold text-[#6F837B] min-w-[30px]">
                                        <?= formatMennyiseg($h['Mennyiseg'] ?? 0) ?>
                                        <?= htmlspecialchars($mertekegyseg) ?>
                                    </span>
                                    <span class="text-gray-700"><?= htmlspecialchars($h['Elnevezes'] ?? '') ?></span>
                                    <?php endif; ?>
                                </li>
                        ...
                        <div>
                            ...
                            <ol class="space-y-6 text-gray-700">
                                ...
                                <li class="flex gap-4 group">
                                    <span
                                        class="w-8 h-8 flex items-center justify-center shrink-0 rounded-full bg-[#6F837B] text-white text-sm font-bold">
                                        <?= $i++ ?>
                                    </span>
                            ...
                            <div class="mt-12 flex justify-end">
                                ...
                                <?php else: ?>
                                <form method="post"
                                    action="receptek.php?id=<?= (int)$receptId ?><?= $aktivSzintParam !== null ? '&szint=' . $aktivSzintParam : '' ?>">
...
(function() {
    ...
    const COLORS = ['#4A7043', '#6F837B', '#95A792', '#FFD166', '#EF476F', '#ffffff', '#06D6A0', '#118AB2',
        '#FFC8DD', '#CAFFBF'
    ];
