...
/* =========================
   ...
   - Tartalmazhat: HTML head, menü, Tailwind importok, közös beállítások
========================= */
include "../head.php";

/* =========================
   ADATBÁZIS KAPCSOLÓDÁS
   - PDO: biztonságos, prepared statement támogatás
   - utf8mb4: ékezetek + emoji kompatibilitás
========================= */
...
/* =========================
   SEGÉDFÜGGVÉNYEK
   - formatIdo: idő megjelenítése "óra/perc" formában
   - formatMennyiseg: 1.00 -> 1, 1.50 -> 1.5 (felesleges 0-k levágása)
========================= */
...
/* =========================
   KÉP ÚTVONAL ÖSSZERAKÁS (DB-ben csak FÁJLNÉV)
   CÉL:
   - DB-ben a Kep mező: csak fájlnév (pl. "AlmasPite.webp")
   - A PHP előállítja a teljes URL-t: /CookQuest/assets/kepek/etelek/ + fájlnév

   MIÉRT JÓ EZ?
   - Nem tárolsz "útvonalat" adatként (könnyebb refaktor, mappaátnevezés)
   - Biztonság: nem lehet DB-ből útvonal-traverzállal trükközni
   - Ha hiányzik a fájl: placeholder képet adunk vissza
========================= */
define('APP_BASE_URL', '/CookQuest'); // projekt gyökér URL (XAMPP alatt jellemzően /CookQuest)
define('RECIPE_IMG_URL', APP_BASE_URL . '/assets/kepek/etelek/'); // képek URL mappája
define('RECIPE_IMG_PLACEHOLDER', 'placeholder.webp'); // ha hiányzik a kép, ezt adjuk vissza

// Fizikai mappa a fájl létezésének ellenőrzéséhez (is_file)
define('RECIPE_IMG_DIR', rtrim($_SERVER['DOCUMENT_ROOT'], '/\\') . RECIPE_IMG_URL);

/**
 * receptKepSrc():
 * - Be: DB-ben tárolt Kep mező (ideálisan fájlnév)
 * - Ki: teljes, biztonságos kép URL (vagy placeholder)
 *
 * Védelem / indoklás:
 * - basename(): ha mégis útvonal lenne a DB-ben, levágjuk és csak fájlnév marad
 * - regex whitelist: csak "normális" fájlnév engedett + képkiterjesztés
 * - is_file(): ha nincs ilyen fájl a mappában, ne törött kép legyen
 * - rawurlencode(): URL-ben biztonságos karakterek
 */
function receptKepSrc(?string $dbKep): string
{
    $dbKep = trim((string)$dbKep);

    // Ha üres, azonnal placeholder
    if ($dbKep === '') {
        return RECIPE_IMG_URL . RECIPE_IMG_PLACEHOLDER;
    }

    // Csak a fájlnév kell (útvonalak levágása)
    $file = basename($dbKep);

    // Whitelist: csak ilyen formátum engedett (nincs ../, nincs szóköz, nincs furcsa karakter)
    if (!preg_match('/^[a-zA-Z0-9._-]+\.(webp|png|jpg|jpeg)$/i', $file)) {
        return RECIPE_IMG_URL . RECIPE_IMG_PLACEHOLDER;
    }

    // Ha a fájl fizikailag nem létezik, placeholder
    if (!is_file(RECIPE_IMG_DIR . $file)) {
        return RECIPE_IMG_URL . RECIPE_IMG_PLACEHOLDER;
    }

    // Kész URL visszaadása
    return RECIPE_IMG_URL . rawurlencode($file);
}

/* =========================
   URL PARAMÉTER KEZELÉSE
   - Ha van ?id=..., akkor egy konkrét receptet mutatunk
   - Ha nincs, listanézet (összes recept)
========================= */
$receptId = isset($_GET['id']) ? (int)$_GET['id'] : null;

/* =========================
   ÖSSZES RECEPT LEKÉRDEZÉSE
   - Listaoldalhoz kell: receptek + kategóriák + nehézség + árkategória
   - ORDER BY: szint, név
========================= */
$receptek = $pdo->query("
    SELECT 
...
/* =========================
   ...
   - Sidebar és fő lista blokkos megjelenítéshez
========================= */
...
/* =========================
   ...
   - Főkategória -> alkategóriák listája checkboxokhoz
========================= */
$kategoriaCheckboxok = [];
foreach ($receptek as $r) {
...
/* =========================
   KONKRÉT RECEPT LEKÉRDEZÉSE (ha van id)
   - Recept részletes adatai
   - Hozzávalók listája (recept_hozzavalo kapcsolótábla)
========================= */
$recept = null;
$hozzavalok = [];
...
<main class="min-h-screen bg-gradient-to-br from-[#9FB1A3] to-[#7F8F83]">
    <div class="max-w-7xl mx-auto py-6 px-4">
        ...
        <!-- =========================
             SZŰRŐ PANEL CSAK LISTA NÉZETBEN
             - Részletes nézetben nem kell (mert ott egy recept van)
        ========================= -->
        <?php if (!$recept): ?>
            <div class="mb-8">
        ...
        <div class="flex flex-col lg:flex-row lg:gap-8">
                        ...
                        <div class="mb-4">
                            <button class="w-full text-left flex items-center justify-between py-2 px-3 bg-gray-50 hover:bg-gray-100 rounded-lg transition szint-sav-cim">
                                <span class="font-semibold text-[#4A7043]">
                                    <?= $szint ?>. szint (<span class="szint-darab" data-szint="<?= $szint ?>"><?= count($lista) ?></span>)
                                </span>
            ...
            <!-- =========================
                 FŐ TARTALOM
                 - Ha nincs id: receptlista
                 - Ha van id: részletes recept oldal
            ========================= -->
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                <?php foreach ($lista as $r): ?>
                                    <a href="receptek.php?id=<?= $r['ReceptID'] ?>"
                                        ...
                                        <!-- KÉP: DB-ből csak fájlnév jön, a PHP állítja elő a teljes útvonalat -->
                ...
                <?php else: ?>
                    ...
                    <div class="bg-white rounded-3xl shadow-2xl overflow-hidden">
                        <div class="relative h-80">
                            ...
                            <!-- KÉP: részletes oldal fejléc képe -->
                        ...
                        <div class="p-8 grid grid-cols-1 lg:grid-cols-[350px_1fr] gap-8">
                            ...
                            <!-- =========================
                                 ELKÉSZÍTÉS MENETE BLOKK
                                 - lépések: Elkeszitesi_leiras sorokra bontása
                            ========================= -->
                            <div>
                                ...
                                <!-- =========================
                                     AKCIÓ GOMB (jelenleg csak UI)
                                     - később ide jöhet: pontok jóváírása, elkészített státusz, stb.
                                ========================= -->
...
<!-- =========================
     KLIENSOLDALI JS (UI: szűrők, sidebar nyit/zár, stb.)
     - A képek útvonalához NEM kell JS, azt PHP már előállítja
========================= -->
